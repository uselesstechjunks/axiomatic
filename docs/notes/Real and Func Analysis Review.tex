...
\title{Real and Functional Analysis Review}
\author{by Aditya Ramdas}
\centering
\date{August 2014}
...
\begin{frame}{Real/functional analysis for ML}
\begin{itemize}
\item Motivation
\item Function Spaces (with examples)
\item Metric Spaces (with examples)
\item Vector Spaces (with examples)
\item Normed Vector Spaces (with examples)
\item Inner Product Space (with examples)
\end{itemize}
\end{frame}

\begin{frame}{Motivation}
\begin{enumerate}
\item Defining and understanding matrix norms, Cauchy-Schwarz, Singular Value Decomposition, etc.
\item Reproducing Kernel Hilbert Spaces
\item Fourier Transform
\item Applications like metric learning
\end{enumerate}
\end{frame}

\begin{frame}{Function Space}
It is a set of functions. Each point in the space is a function.
\begin{enumerate}
\item $C[a, b]$ - continuous  functions on $[a, b]$
\item $L_1[a,b]$ - $\{f: \int_a^b |f(x)| dx < \infty\}$
\item $L_2[a,b]$ - $\{f: \int_a^b |f(x)|^2 dx < \infty\}$
\end{enumerate}
\end{frame}

\begin{frame}{Metric Space}
It is a pair $(\chi,\rho)$ where $\chi$ is a space, $\rho$ is a meric. A distance metric is any function statisfying
\begin{enumerate}
\item $\rho(a,b)\in\mathbb{R}_0^+$ for all $a,b\in\chi$
\item $\rho(a,b)=\rho(b,a)$
\item $\rho(a,b)=0$ iff $a=b$
\item $\rho(a,c)\le \rho(a,b)+\rho(b,c)$
\end{enumerate}
\end{frame}

\begin{frame}{Metric Space (examples)}
\begin{enumerate}
\item $(\mathbb{R},|y-x|)$
\item $(\mathbb{R}^+,|y-x|)$
\item $(\mathbb{R}^+,|\ln(y/x)|)$
\item $\left( L_2(-\infty,\infty),\sqrt{\int_{-\infty}^\infty (f_1(x)-f_2(x))^2 dx}\right)$
\item $(\mathbb{R}^{m\times n},\text{rank}(Y-X))$
\item $($String,Levenshtein/editdistance$)$
\item $($Graph,Shortestpathdistance$)$
\end{enumerate}
\end{frame}
