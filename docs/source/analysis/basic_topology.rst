...
.. note::
    * **Metric Space**: A set :math:`X` is a metric space if there exists a function :math:`d : X \times X \to \mathbb{R}_0^{+}` such that for any :math:`p,q,r \in X`:

        #. :math:`d(p,q) = 0 \Longleftrightarrow p = q`
        #. :math:`d(p,q) = d(q,p)`
...
.. note::
    * **k-cells in :math:`\mathbb{R}^k`**: For :math:`a_i,b_i \in \mathbb{R}` with :math:`a_i < b_i` for all :math:`i \in \{1,\dots,k\}`, a :math:`k`-cell is

        .. math:: \{x \mid a_i \le x_i \le b_i\}

      where :math:`x = (x_1,\cdots,x_k)^{T} \in \mathbb{R}^k`.
...
.. note::
    * **Open/Closed-Balls in :math:`\mathbb{R}^k`**: For :math:`x \in \mathbb{R}^k`, an open ball (or closed ball) centered at :math:`x` with radius :math:`r>0` is
        ...
        .. math:: \bar{B}_r(x) := \{y \in \mathbb{R}^k \mid |x-y| \le r\}.
...
In all the following definitions, :math:`X` is a metric space.
...
.. note::
    * **Bounded Set**: :math:`E \subseteq X` is bounded iff :math:`\exists p \in X` and :math:`\exists M>0` such that :math:`E \subseteq N_M(p)`.
...
.. note::
    * **Interior Points**: :math:`p \in E` is an interior point of :math:`E` iff :math:`\exists r>0` such that :math:`N_r(p) \subseteq E`.

.. tip::
    Interior point of a set :math:`E` (which has to be inside :math:`E`) means we have a little breathing space to step outside of that point without crossing the border of :math:`E`.

.. remark::
    The set of all limit points of a set :math:`E \subseteq X` is denoted :math:`E^{\uparrow}`.

.. remark::
    The set of all interior points of a set :math:`E \subseteq X` is denoted :math:`E^{0}`.

.. note::
    * **Isolated Points**: All points :math:`p \in E` that are not limit points of :math:`E` (:math:`p \in E \wedge p \notin E^{\uparrow}`) are isolated points of :math:`E`.

.. tip::
    Can't get close to those points without crossing :math:`E`'s border.

.. note::
    * **Closed Set**: If :math:`E \subseteq X` contains all of its own limit points (:math:`p \in E^{\uparrow} \implies p \in E`) it is closed.
...
.. remark::
    A set that has no limit points (:math:`E^{\uparrow}=\varnothing`) is closed by definition.
...
.. note::
    * **Open Set**: If all points of :math:`E` are interior points, then :math:`E` is open. Symbolically,

        .. math:: p \in E \Longrightarrow p \in E^{0}.
...
.. note::
    * **Closure**: The set
        ...
        .. math:: E^{\circ} := E \cup E^{\uparrow}

      is the closure of :math:`E`.

.. note::
    * **Dense Set**: A set :math:`E` is dense in :math:`X` iff every point in :math:`X` lies within the closure of :math:`E` (:math:`p \in X \Longrightarrow p \in E^{\circ}`).
...
.. note::
    ...
    * **Theorem**: :math:`F` is closed :math:`\Longleftrightarrow F^{c}` is open.
    ...
    * **Theorem**: For :math:`E \subseteq X`:
        ...
        #. :math:`E^{\circ}` is closed.
        #. :math:`E = E^{\circ}` iff :math:`E` is closed.
        #. For every closed set :math:`F \subseteq X`, :math:`E \subseteq F \Longrightarrow E^{\circ} \subseteq F`.

    * **Theorem**: Let :math:`E` be a nonempty set of real numbers which is bounded above. Let :math:`y=\sup E`. Then :math:`y \in E^{\circ}` (and :math:`y \in E`, if :math:`E` is closed).

.. note::
    * **Relatively Open**: Suppose :math:`E \subset Y \subset X`. :math:`E` is open relative to :math:`Y` if :math:`\forall p \in E`, :math:`\exists r>0` such that :math:`\exists q \in Y` with :math:`d(p,q) < r \Longrightarrow q \in E`.

.. note::
    * **Example**: A set :math:`E` can be relatively open to :math:`Y` without being open in :math:`X`. For example, the segment :math:`(a,b)` is not open in :math:`\mathbb{R}^{2}` (any neighbourhood defined on :math:`\mathbb{R}^{2}` around a point in the segment ought to contain points from the orthogonal direction - which are not in the segment). However, it is relatively open to :math:`\mathbb{R}`.

.. note::
    * **Theorem**: For :math:`E \subset Y \subset X`, :math:`E` is open relative to :math:`Y \Longleftrightarrow E = Y \cap G` for some open set :math:`G \subset X`.
...
.. note::
    * **Compactness**: :math:`K \subseteq X` is compact iff every open cover of :math:`K` contains a finite sub-cover. Formally, for any open cover of :math:`K`, :math:`\{G_{\alpha}\}`, :math:`\exists \alpha_1,\cdots,\alpha_n` for some :math:`n>0` such that

        .. math:: K \subset \bigcup_{i=1}^{n} G_{\alpha_i}.

.. note::
    * **Sequential Compactness**: :math:`K \subset X` is sequentially compact if every infinite subset :math:`E \subset K` has a limit point in :math:`K`, or alternatively, every sequence in :math:`K` has a subsequence which converges to some point in :math:`K`.

Properties Related to General Metric Space
================================================================================

.. note::
    * **Theorem**: For :math:`K \subset Y \subset X`, :math:`K` is compact relative to :math:`X \Longleftrightarrow K` is compact relative to :math:`Y`.
...
.. note::
    ...
    * **Corollary**: If :math:`F \subset X` is closed and :math:`K \subset X` is compact then :math:`F \cap K` is compact.
    ...
    * **Corollary**: For a sequence of non-empty compact sets :math:`\{K_n\}`, :math:`\forall n>0,\ K_n \supset K_{n+1} \Longrightarrow \bigcap_{i=1}^{\infty} K_i \ne \varnothing`.
    * **Theorem**: For metric spaces, compactness :math:`\Longleftrightarrow` sequential-compactness.

Properties Related to Euclidean Space
================================================================================

.. note::
    * **Theorem** (:math:`\mathbb{R}`): For a sequence of intervals :math:`\{I_n\}` such that :math:`I_n \supset I_{n+1}` for all :math:`n>0`, we have :math:`\bigcap_{i=1}^{\infty} I_i \ne \varnothing`.

.. note::
    * **Theorem** (:math:`\mathbb{R}^k`): For a sequence of :math:`k`-cells :math:`\{I_n\}` such that :math:`I_n \supset I_{n+1}` for all :math:`n>0`, we have :math:`\bigcap_{i=1}^{\infty} I_i \ne \varnothing`.
...
.. note::
    * **Separated Sets**: :math:`A,B \subset X` are separated if
        ...
        .. math:: A \cap B^{\circ} = \varnothing \ \wedge\ A^{\circ} \cap B = \varnothing.
...
.. remark::
    Separated :math:`\Longrightarrow` disjoint. Converse does not hold. For example, :math:`[0,1]` and :math:`(1,2)` are disjoint but not separated.

.. note::
    * **Theorem**: For a set :math:`E \subset \mathbb{R}`,

        .. math:: E \text{ is connected} \Longleftrightarrow \bigl(\forall x,y \in E,\ (x<z<y) \Longrightarrow z \in E\bigr).
